...
router.get('/', requireAuth('ADMIN'), async (req, res) => {
  ...
  try { const { balanceMap } = await computeBalances(); const withComputed = clients.map(c => ({ ...c, currentBalanceComputed: balanceMap.get(c.id) || c.currentBalance })); return res.json(withComputed); } catch (e) { return res.json(clients); }
});

router.post('/', requireAuth('ADMIN'), async (req, res) => {
  const { name, email, password, startDate, initialInvestment } = req.body || {};
  if (!name || !email || !password || !startDate || initialInvestment == null) {
  ...
  if (score < 3) {
    return res.status(400).json({ error: 'Senha fraca. Exija ao menos 8 caracteres e combine tipos.' });
  ...
  try {
    ...
    res.status(400).json({ error: 'NÃ£o foi possÃ­vel criar cliente', detail: e.message });
  }
});
...
router.get('/me', requireAuth(), async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: 'Não foi possível obter cliente', detail: e.message });
  }
});
...
router.get('/me/summary', requireAuth(), async (req, res) => {
  if (req.user.role !== 'CLIENT') return res.status(403).json({ error: 'Forbidden' });
  try {
    const summary = await getClientSummary(req.user.sub, { last: Number(req.query.last) || 10 });
    return res.json(summary);
  } catch (e) {
    return res.status(400).json({ error: 'Não foi possível obter resumo', detail: e.message });
  }
});
