...
function generateExecutiveReport(month = null) {
  try {
    ...
    const totalInitial = systemData.clients.reduce(
      (sum, c) => sum + c.initialInvestment,
      0
    );
    const totalProfit = totalCapital - totalInitial;
    const profitability =
      totalInitial > 0 ? (totalProfit / totalInitial) * 100 : 0;

    // Filtra operações do mês
    const monthOperations = month
      ? systemData.operations.filter((op) => op.date.startsWith(month))
      : systemData.operations.filter((op) =>
          op.date.startsWith(new Date().toISOString().substr(0, 7))
        );

    const monthlyReturn = monthOperations.reduce(
      (sum, op) => sum + op.result,
      0
    );
    ...
    const successRate =
      monthOperations.length > 0
        ? (positiveOps / monthOperations.length) * 100
        : 0;
    ...
    const summaryData = [
      {
        label: "Capital Total Gerenciado:",
        value: formatCurrency(totalCapital),
      },
      {
        label: "Total de Clientes Ativos:",
        value: systemData.clients.length.toString(),
      },
      {
      ...
      { label: "Taxa de Sucesso:", value: `${successRate.toFixed(1)}%` },
      {
        ...
        value: `${profitability >= 0 ? "+" : ""}${profitability.toFixed(2)}%`,
      },
    ];
    ...
    if (tableData.length > 0) {
      ...
      pdf.setFontSize(10);
      pdf.text(
        "Nenhuma operação encontrada neste período.",
        20,
        yPosition + 10
      );
    }
    ...
    topClients.forEach((client, index) => {
      ...
      pdf.text(
        `${clientProfitability >= 0 ? "+" : ""}${clientProfitability.toFixed(
          2
        )}%`,
        165,
        yPosition + 4
  ...
  try {
    ...
    const clientData = [
      { label: "Data de Início:", value: formatDate(client.startDate) },
      {
        ...
        value: formatCurrency(client.initialInvestment),
      },
    ...
    clientData.forEach((item) => {
      ...
      if (
        ...
        item.label.includes("Rentabilidade")
      ) {
        if (profitability >= 0) {
          pdf.setTextColor(
            colors.success[0],
    ...
    if (clientOperations.length > 0) {
      ...
      clientOperations.forEach((op, index) => {
        ...
        const description =
          op.description.length > 35
            ? op.description.substr(0, 32) + "..."
            : op.description;
    ...
    pdf.text(
      `Gerado em ${new Date().toLocaleString("pt-BR")} para ${client.name}`,
      15,
      290
    );

    // ===== SALVAR PDF =====
    const fileName = `CIMA-Relatorio-${client.name.replace(/\s+/g, "-")}-${
...
function generateOperationsReport() {
  try {
    ...
    pdf.setFontSize(10);
    pdf.text(
      `Total de ${systemData.operations.length} operações registradas`,
      20,
      28
    );
    ...
    // ===== ESTATÍSTICAS GERAIS =====
    const totalReturn = systemData.operations.reduce(
      (sum, op) => sum + op.result,
      0
    );
    const positiveOps = systemData.operations.filter(
      (op) => op.result > 0
    ).length;
    const negativeOps = systemData.operations.filter(
      (op) => op.result < 0
    ).length;
    const avgReturn = totalReturn / systemData.operations.length;
    const successRate = (positiveOps / systemData.operations.length) * 100;

    const bestOp = systemData.operations.reduce((max, op) =>
      op.result > max.result ? op : max
    );
    const worstOp = systemData.operations.reduce((min, op) =>
      op.result < min.result ? op : min
    );
    ...
    const statsData = [
      ...
      {
        label: "Retorno Médio por Operação:",
    ...
    systemData.operations
      ...
      .forEach((op, index) => {
        ...
        const bgColor =
          operationCount % 2 === 0 ? [255, 255, 255] : colors.light;
        pdf.setFillColor(...bgColor);
        pdf.rect(15, yPosition, 180, 6, "F");
    ...
    const fileName = `CIMA-Operacoes-Completo-${
      new Date().toISOString().split("T")[0]
    }.pdf`;
    pdf.save(fileName);
...
function showPDFWarning() {
  if (!checkPDFAvailability()) {
    showAlert(
      "Biblioteca jsPDF não encontrada. Funcionalidade de PDF indisponível.",
      "warning"
    );
    return false;
  }
