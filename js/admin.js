...
function showAdminTab(tabName) {
  ...
  if (tabName === "overview") {
    ...
    // Define data padrão para o formulário de operações
    document.getElementById("operationDateOps").value = new Date()
      .toISOString()
      .split("T")[0];
...
function updateAdminOverview() {
  ...
  const monthProfit = (systemData.operations || [])
    ...
    .reduce(
      (sum, op) => sum + ((op.result || 0) / 100) * (op.totalCapital || 0),
      0
    );
...
function addOperation() {
  ...
  if (Math.abs(result) > 50) {
    if (
      !confirm(`Resultado de ${result}% é muito alto. Confirma este valor?`)
    ) {
      return;
    }
  }
  ...
  if (operationDate > maxDate) {
    showAlert(
      "Data da operação não pode ser mais de 1 dia no futuro!",
      "danger"
    );
    return;
  }
  ...
  if (operationDate < minDate) {
    showAlert(
      "Data da operação não pode ser mais de 90 dias no passado!",
      "danger"
    );
    return;
  }
  ...
  // Verifica duplicatas
  const existingOperation = systemData.operations.find(
    (op) =>
      op.date === date &&
  ...
  if (existingOperation) {
    if (
      !confirm("Já existe uma operação similar nesta data. Deseja continuar?")
    ) {
      return;
    }
  }
  ...
  if (hasToken && systemData.userType === "admin" && window.CIMA_API) {
    (async () => {
      try {
        await window.CIMA_API.createOperation({
          date,
          description,
          resultPct: result,
        });
        ...
        updateAdminOverview();
        showAlert(
          `Operação registrada no servidor com sucesso! Resultado: ${result.toFixed(
            2
          )}%`,
          "success"
        );
        ...
        showAlert(
          "Falha ao registrar operação no servidor. Verifique a conexão.",
          "danger"
        );
      }
    })();
  ...
  const totalCapital = systemData.clients.reduce((sum, client) => {
    ...
    return (
      sum +
      (!clientStart || clientStart <= operationDate ? client.currentBalance : 0)
    );
  }, 0);
    ...
    window.CIMA_API.getOperations()
      .then((ops) => {
        const mapped = (ops || []).map((op) => ({
          ...
          totalCapital:
            op.totalCapital != null ? parseFloat(op.totalCapital) : null,
        }));
        mapped
          .slice()
          .reverse()
          .forEach((operation, index) => {
            const hasCapital =
              operation.totalCapital != null && !isNaN(operation.totalCapital);
            const impactVal = hasCapital
              ? (operation.result / 100) * operation.totalCapital
              : null;
            const impactCell = hasCapital
              ? `<strong>${formatCurrency(impactVal)}</strong>`
              : "<strong>-</strong>";
            const row = document.createElement("tr");
            row.innerHTML = `
      ...
      .catch((err) => {
        console.warn(
          "Falha ao carregar operações da API, usando dados locais:",
          err && err.message
        );
        renderLocalOps();
      });
...
function addOperationFromOpsTab() {
  const date = document.getElementById("operationDateOps").value;
  const resultInput = document.getElementById("operationResultOps").value;
  const description = sanitizeInput(
    document.getElementById("operationDescOps").value
  );
  ...
  // Se autenticado como admin, registra operação na API
  const hasToken = !!localStorage.getItem("cimaAccessToken");
  if (hasToken && systemData.userType === "admin" && window.CIMA_API) {
    (async () => {
      try {
        ...
        // Clear form
        document.getElementById("operationDateOps").value = "";
        document.getElementById("operationResultOps").value = "";
        document.getElementById("operationDescOps").value = "";
        ...
        updateAdminOverview();
        showAlert(
          `Operação "${description}" registrada com sucesso! Resultado: ${result.toFixed(
            2
          )}%`,
          "success"
        );
  ...
  const resultText =
    result >= 0 ? `+${result.toFixed(2)}%` : `${result.toFixed(2)}%`;
  showAlert(
    `Operação "${description}" registrada com sucesso! Resultado: ${resultText}`,
    "success"
  );
}
