...
async function initializeSystem() {
  try {
    ...
    // Configura formularios de operacoes
...
async function migratePasswordsToHash() {
  ...
  try {
    ...
    for (let client of systemData.clients) {
      if (client.password && client.password.length < 20) {
        // Senhas não hasheadas são curtas
        // Para dados de exemplo, usa senha padrão
        if (client.email === "joao@email.com") {
          client.password = await hashPassword("123456");
        } else {
    ...
    try {
      ...
      if (now - session.timestamp < 8 * 60 * 60 * 1000) {
        if (session.userType === "admin") {
          ...
          const client = systemData.clients.find(
...
function refreshCharts() {
  ...
  if (
    ...
    window.performanceChartInstance
  ) {
...
function exportToJSON(timestamp) {
  const exportData = {
    ...
    summary: {
      ...
      totalCapital: systemData.clients.reduce(
...
function verifyDataIntegrity() {
  ...
  systemData.operations.forEach((operation, index) => {
    ...
    if (Math.abs(operation.result) > 100) {
      issues.push(
        `Operação ${index + 1}: Resultado improvável (${operation.result}%)`
      );
    }
  });
...
function measurePerformance(action, fn) {
  ...
  performanceMetrics[action] =
    (performanceMetrics[action] || 0) + (end - start);

  if (end - start > 1000) {
    // Log operações lentas (>1s)
...
function cleanupOldData() {
  ...
  const oldOperations = systemData.operations.filter(
    (op) => new Date(op.date) < oneYearAgo
  );
