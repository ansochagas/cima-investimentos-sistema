...
function showClientDashboard(client) {
    ...
    document.getElementById('clientName').innerHTML = `<i class="fas fa-user-tie"></i> ${client.name}`;
    document.getElementById('clientInitialInvestment').textContent = formatCurrency(client.initialInvestment);
    document.getElementById('clientCurrentBalance').textContent = formatCurrency(client.currentBalance);
    ...
    document.getElementById('clientTotalProfit').textContent = formatCurrency(profit);
    document.getElementById('clientProfitability').textContent = profitability + '%';
            ...
            .then((summary) => {
                try {
                    const ops = (summary.lastOperations || []).map(o => ({
                        ...
                        impact: typeof o.impact === 'string' ? parseFloat(o.impact) : o.impact,
                    }));
                    updateClientOperationsTableAPI(client, ops);
                } catch (e) {
                    console.warn('Falha ao processar resumo do cliente. Usando fallback local.', e);
                    updateClientOperationsTable(client);
                }
            })
            .catch((err) => {
                console.warn('Falha ao carregar resumo do cliente. Usando fallback local.', err && err.message);
                updateClientOperationsTable(client);
            });
    } else {
        updateClientOperationsTable(client);
...
function updateClientOperationsTable(client) {
    ...
    const totalCurrentCapital = systemData.clients.reduce((sum, c) => sum + c.currentBalance, 0);
    const clientProportion = client.currentBalance / totalCurrentCapital;
    
    systemData.operations.slice(-10).reverse().forEach(operation => {
        const impact = (operation.result / 100) * (operation.totalCapital * clientProportion);
